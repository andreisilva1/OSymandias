...
@app.command()
def init(
    example: bool = typer.Option(False, "--example", help="Also scaffold a runnable example agent + tool."),
):
    ...
    # Optional runnable example (agent + tool), auto-discovered by `osy serve`.
    if example:
        example_dest = cwd / "example_agent.py"
        if example_dest.exists():
            console.print("[yellow]↳ example_agent.py already exists, skipping[/yellow]")
        else:
            _write_example_agent(example_dest)
            console.print("[green]✓[/green] example_agent.py created")

    console.print("\n[bold green]Ready.[/bold green] Run [cyan]osy serve[/cyan] to start.")
    if example:
        console.print('[dim]Then try:[/dim] [cyan]osy submit "Summarise: OSymandias is a multi-agent runtime." --watch[/cyan]')
    console.print()
...
def _write_example_agent(dest: Path) -> None:
    dest.write_text('''\
# example_agent.py — runnable example generated by `osy init --example`
#
# Defines one tool and one agent. `osy serve` auto-discovers both, so after
# starting the runtime you can submit a job that routes to this agent:
#
#   osy submit "Summarise: OSymandias is a multi-agent runtime." --watch
...
@osy.agent("EchoAgent", description="Echoes the task back with a word count.")
def echo(task: str, ctx: OsyContext) -> dict:
    """A minimal agent: reports the word count of its task and echoes it back."""
    ctx.emit_event("TASK_PROGRESS", {"step": "counting"})
    wc = word_count(task)["words"]
    return {"summary": task, "word_count": wc}
''', encoding="utf-8")
