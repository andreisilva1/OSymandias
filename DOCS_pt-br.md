...
> **Webhook de lifecycle vs `webhook_url` de tool — são coisas diferentes.**
> Um **webhook de lifecycle** (esta página / `/api/v1/webhooks`) é *saída*: o OSymandias faz POST na sua URL **toda vez que um job termina** (completed/failed/cancelled/budget exceeded), independente do prompt. Use pra notificar Slack, disparar um fluxo n8n, atualizar um dashboard, etc.
> O **`webhook_url`** de uma tool (no Syscall Registry / `@osy.tool`) é o oposto — *execução de entrada*: é o endpoint que o OSymandias **chama pra executar aquela tool** durante um job. Pra fazer um agente "fazer X e mandar pro Slack" sob demanda, registre uma tool de Slack com `webhook_url` (ex.: uma incoming webhook do Slack); o agente a chama quando a tarefa pede. Webhooks de lifecycle não são prompt-driven.
