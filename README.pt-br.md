...
- **Aprovação humana (human-in-the-loop)** — marque um **agente** ou uma **task** como `requires_approval`; aguarda em `HUMAN_REVIEW` até aprovar. Uma inbox **Approvals** cross-job lista tudo que está pendente.
...
```bash
# Pelo terminal — submeta um objetivo e acompanhe ao vivo
osy submit "pesquise o mercado de EV e escreva um relatório" --max-tokens 50000 --watch
```
