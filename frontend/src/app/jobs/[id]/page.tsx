...
export default function ProcessDetailPage({ params }: { params: { id: string } }) {
  ...
  const { data: tasks = [] } = useJobTasks(params.id);
  const { data: toolCalls = [] } = useJobToolCalls(params.id);
  const { data: messages = [] } = useJobMessages(params.id);
  const { data: agentInstances = [] } = useJobAgentInstances(params.id);
  const { events } = useJobStream(params.id);
